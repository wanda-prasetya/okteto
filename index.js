...
require("./resta")(Resta, m, chatUpdate, store)
} catch (err) {
console.log(err)
}
})
...
Resta.ev.on('groups.update', async pea => {
    ...
    // Get Profile Picture Group
       try {
       ppgc = await Resta.profilePictureUrl(pea[0].id, 'image')
       } catch {
       ppgc = 'https://telegra.ph/file/3983c55ac7f3ebea225d3.jpg'
       }
       let wm_fatih = { url : ppgc }
       if (pea[0].announce == true) {
       Resta.send5ButImg(pea[0].id, `「 Group Settings Change 」\n\nGroup telah ditutup oleh admin, Sekarang hanya admin yang dapat mengirim pesan !`, `Group Settings Change Message`, wm_fatih, [])
...
// Setting
    ...
    Resta.ev.on('connection.update', async (update) => {
        ...
        let reason = lastDisconnect.error ? lastDisconnect?.error?.output.statusCode : 0;
            ...
            else if (reason === DisconnectReason.connectionClosed) { console.log("Connection closed, reconnecting...."); startResta(); }
            else if (reason === DisconnectReason.connectionLost) { console.log("Connection Lost from Server, reconnecting..."); startResta(); }
            else if (reason === DisconnectReason.connectionReplaced) { console.log("Connection Replaced, Another New Session Opened, Please Close Current Session First"); process.exit(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`Device Logged Out, Please Delete Session and Scan Again.`); process.exit(); }
            else if (reason === DisconnectReason.restartRequired) { console.log("Restart Required, Restarting..."); startResta(); }
            else if (reason === DisconnectReason.timedOut) { console.log("Connection TimedOut, Reconnecting..."); startResta(); }
     ...
     * @returns
     */
    Resta.send5ButImg = async (jid , text = '' , footer = '', img, but = [], thumb, options = {}) =>{
        let message = await prepareWAMessageMedia({ image: img, jpegThumbnail:thumb }, { upload: Resta.waUploadToServer })
     ...
     * @returns 
     */
    Resta.sendTextWithMentions = async (jid, text, quoted, options = {}) => Resta.sendMessage(jid, { text: text, contextInfo: { mentionedJid: [...text.matchAll(/@(\d{0,16})/g)].map(v => v[1] + '@s.whatsapp.net') }, ...options }, { quoted })
    ...
    Resta.sendFile = async(jid, PATH, fileName, quoted = {}, options = {}) => {
        let types = await Resta.getFile(PATH, true)
        ...
        await Resta.sendMessage(jid, { [type]: { url: pathFile }, mimetype, fileName, ...options }, { quoted, ...options })
        return fs.promises.unlink(pathFile)
    }
    Resta.parseMention = async(text) => {
